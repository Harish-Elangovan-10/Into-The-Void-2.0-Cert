...
import SandboxCertificateTemplate from "@/components/SandboxCertificateTemplate";
...
export default function SandboxCertificatePage() {
  const router = useRouter();
  const [attendee, setAttendee] = useState<Attendee | null>(null);
  const [downloading, setDownloading] = useState(false);

  useEffect(() => {
    const sessionData = sessionStorage.getItem("sandbox_participant");
    if (!sessionData) {
      router.push("/sandbox_workshop");
    } else {
  ...
  const handleLogout = () => {
    sessionStorage.removeItem("sandbox_participant");
    router.push("/sandbox_workshop");
  };
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      pdf.save(`Sandbox_Workshop_2026_Certificate_${attendee.name.replace(/\s+/g, "_")}.pdf`);
    } catch (error) {
  ...
  return (
    <main className="flex-1 flex flex-col items-center max-w-5xl mx-auto w-full p-6 space-y-6 animate-fade-in">
      ...
      <div className="w-full flex flex-col md:flex-row items-center justify-between gap-4 border-b border-white/5 pb-6">
        <div className="flex items-center gap-4">
          ...
          <div className="text-left font-mono">
            <h1 className="text-sm font-bold text-text uppercase tracking-wider">
              Sandbox Workshop Verification Node
            </h1>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <button
            ...
            className="px-5 py-2.5 bg-gradient-to-r from-blue-600 to-blue-400 border border-white/10 text-white font-mono text-xs font-semibold uppercase tracking-wider rounded-lg flex items-center gap-2 hover:border-white/20 active:scale-95 transition-all duration-200 cursor-pointer"
          >
        ...
        <AnimatedBorder className="w-full">
          <div className="p-4 md:p-8 bg-[#0D111B]/80 backdrop-blur-md max-lg:overflow-x-auto scrollbar-thin scrollbar-thumb-white/10">
            <div className="w-full flex justify-center">
              <SandboxCertificateTemplate attendee={attendee} />
            </div>
          </div>
        </AnimatedBorder>
      </div>
      ...
      <div className="w-full font-mono text-[10px] text-text-dim/80 space-y-1 bg-[#0D111B]/30 border border-white/5 rounded-lg p-4">
        ...
        <p>&gt; EVENT: OPEN_SANDBOX_DEVELOPMENT_WORKSHOP_JULY_2026</p>
